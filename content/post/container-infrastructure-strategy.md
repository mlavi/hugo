...
Principal Architect at CoreOS, but I only found [a partial reference](https://www.youtube.com/watch?v=P3sO9URqOhE)
...
persistent infrastructure (which is longer lived).
...
but use [Packer to exclusively build your container artifact](https://packer.io/docs/builders/docker.html).
...
unless you have some sort of external source driving it.

Containers in production are easy to deploy when used for the single tier of a web application which
...
versions of your API services (if necessary when you do not design forward and backward compatible APIs).

But the orchestration of a complex roll out of an entire service upgrade representing a medley of
